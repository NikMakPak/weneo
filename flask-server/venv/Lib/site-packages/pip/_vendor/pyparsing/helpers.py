...
def nested_expr(
    ...
    ref: typing.Optional[ParserElement] = None,
    ...
    *,
    ...
    - ``closer`` - closing character for a nested list
      (default= ``")"``); can also be a pyparsing expression
    - ``ref`` - expression for items within the nested lists
      (default= ``None``)
    ...
    If an expression is not provided for the ref argument, the
    nested expression will capture all whitespace-delimited ref
    ...
    if ref is None:
        if isinstance(opener, str_type) and isinstance(closer, str_type):
            ...
            raise ValueError(
                "opening and closing arguments must be strings if no ref expression is given"
            )
    ret = Forward()
    if ignoreExpr is not None:
        ret <<= Group(
            Suppress(opener) + ZeroOrMore(ignoreExpr | ret | ref) + Suppress(closer)
        )
    else:
        ret <<= Group(Suppress(opener) + ZeroOrMore(ret | ref) + Suppress(closer))
