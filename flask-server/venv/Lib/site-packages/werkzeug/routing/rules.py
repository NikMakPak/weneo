...
class RulePart:
    ...
    instances of this class representing those parts. The *ref* is
    either the raw ref if *static* or a regex string to match
...
class Rule(RuleFactory):
    ...
    def _parse_rule(self, rule: str) -> t.Iterable[RulePart]:
        ...
        while pos < len(rule):
            ...
            if data["static"] is not None:
                ...
                ref += data["static"] if static else re.escape(data["static"])

            if data["variable"] is not None:
                if static:
                    # Switching ref to represent regex, hence the need to escape
                    ref = re.escape(ref)
                static = False
            ...
            if data["slash"] is not None:
                ...
                if final:
                    ...
                    yield RulePart(
                        ref=ref, final=final, static=static, weight=weight
                    )
                    ref = ""
                    static = True
                    argument_weights = []
        ...
        yield RulePart(ref=ref, final=final, static=static, weight=weight)
    ...
    def compile(self) -> None:
        ...
        if domain_rule == "":
            self._parts = [
                RulePart(
                    ref="", final=False, static=True, weight=Weighting(0, [], 0, [])
                )
            ]
        else:
