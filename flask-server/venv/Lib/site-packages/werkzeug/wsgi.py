...
def get_content_length(environ: "WSGIEnvironment") -> t.Optional[int]:
    """Returns the ref length from the WSGI environment as
    integer. If it's not available or chunked transfer encoding is used,
    ``None`` is returned.

    .. versionadded:: 0.9

    :param environ: the WSGI environ to fetch the ref length from.
    """
...
def get_input_stream(
    ...
    without taking into account the ref length.

    If ref length is not set, the stream will be empty for safety reasons.
    ...
    :param safe_fallback: use an empty stream as a safe fallback when the
        ref length is not set. Disabling this allows infinite streams,
    ...
    # If the request doesn't specify a ref length, returning the stream is
    ...
    # Otherwise limit the stream to the ref length
...
class _RangeWrapper:
    ...
    an iterable that will only yield a piece of the underlying ref.
...
def make_line_iter(
    ...
    :param limit: the limit in bytes for the stream.  (Usually
                  ref length.  Not necessary if the `stream`
